...
![EA Diagram](/docs/images/EA_Diagram.png "EA Diagram")
...
## Validator
De maneira a facilitar a deteção de erros e aumentar a segurança durante a inserção de dados pelo utilizador (impedir, por exemplo, ataques XSS), foi criada a classe *Validator*, que será utilizada nos Handlers dos métodos POST e PUT. Esta classe contém apenas dois métodos:

* `validateString`, que verifica uma dada String de acordo com as seguintes condições:
    ...
    * A não existência de caracteres utilizados em HTML (<>).
    ...
    Retorna `true` caso a validação tenha sucesso, caso contrário é lançada uma exceção do tipo *ValidationException*, sendo que para lançar a mesma, é necessário passar-lhe o nome da String validada, assim como uma mensagem sobre o erro de validação.

* `validateDateMultipleOf`, que verifica se os minutos de uma dada data são múltiplos de um dado número. Retorna `true` caso isso se verifique, caso contrário, é também lançada uma *ValidationException*, passando-lhe o nome da data, assim como uma mensagem informativa sobre o erro.

A razão pela qual é passado o nome do parâmetro deve-se ao facto de possibilitar a passagem do nome do campo que falhou a validação durante a execução de um comando, a um outro comando por parâmetro. Isto faz com que seja possível, por exemplo, mostrar uma mensagem de erro ao utilizador num formulário HTML, mesmo por baixo do campo em questão.
...
* validatedParamName: O nome do parâmetro que foi alvo de validação.
